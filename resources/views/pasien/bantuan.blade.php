...
<body class="font-sans antialiased text-slate-800 bg-white min-h-screen flex flex-col">
    ...
    <main class="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <div class="max-w-3xl mx-auto">
            <div class="text-center mb-10">
                ...
                <p class="text-slate-500 text-lg">Jika Anda mengalami sakit gigi hebat, nyeri tidak tertahan, atau
                    darurat gigi lainnya, segera tekan layanan darurat 24 jam di bawah.</p>
            </div>

            <div class="max-w-md mx-auto gap-6">
                <!-- Resepsionis -->
                <div
                    class="bg-white border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] rounded-3xl p-8 hover:shadow-lg transition-transform hover:-translate-y-1">
                    ...
                    <p class="text-slate-500 mb-6 text-sm">Gagal login? Salah input data KTP? Konsultasikan keluhan
                        aplikatif dan jadwal Anda ke admin.</p>
                    <div x-data="{ showWaModal: false }">
                        <button type="button" @click="showWaModal = true"
                            class="block text-center w-full py-3.5 px-4 border border-blue-200 bg-blue-50 text-primary font-bold rounded-xl hover:bg-blue-100 transition-colors">Chat
                            Admin Pendaftaran</button>
                        ...
                        <template x-teleport="body">
                            <div x-show="showWaModal" x-transition.opacity
                                ...
                                <div @click.away="showWaModal = false"
                                    class="bg-white rounded-3xl p-6 md:p-8 max-w-sm w-full shadow-2xl relative">
                                    <button @click="showWaModal = false"
                                        class="absolute top-4 right-4 text-slate-400 hover:text-slate-600 font-bold">&times;</button>
                                    <h3 class="font-bold text-xl text-slate-800 mb-2">Pilih Cabang Admin</h3>
                                    <p class="text-sm text-slate-500 mb-6">Pilih admin cabang mana yang ingin Anda
                                        hubungi via WhatsApp:</p>
                                    ...
                                    @endphp
                                    <div class="space-y-3">
                                        <a href="https://wa.me/{{ $settingWa->wa_tegal ?? '6281234567890' }}?text={{ $waTextTegal }}"
                                            target="_blank"
                                            class="flex items-center gap-3 p-4 border border-slate-200 rounded-2xl hover:border-blue-500 hover:bg-blue-50 transition-colors group">
                                            <div
                                            ...
                                            <div>
                                                <h4 class="font-bold text-slate-700 group-hover:text-blue-700">Admin
                                                    Tegal</h4>
                                            </div>
                                        </a>
                                        <a href="https://wa.me/{{ $settingWa->wa_slawi ?? '6281234567891' }}?text={{ $waTextSlawi }}"
                                            target="_blank"
                                            class="flex items-center gap-3 p-4 border border-slate-200 rounded-2xl hover:border-blue-500 hover:bg-blue-50 transition-colors group">
                                            <div
                                                class="w-10 h-10 rounded-full bg-green-100 text-green-600 flex items-center justify-center">
                                        ...
                                        <a href="https://wa.me/{{ $settingWa->wa_brebes ?? '6281234567892' }}?text={{ $waTextBrebes }}"
                                            target="_blank"
                                            class="flex items-center gap-3 p-4 border border-slate-200 rounded-2xl hover:border-blue-500 hover:bg-blue-50 transition-colors group">
                                            <div
                                                class="w-10 h-10 rounded-full bg-green-100 text-green-600 flex items-center justify-center">
            ...
            <div class="mt-12 bg-white p-6 md:p-10 rounded-3xl border border-slate-100 shadow-sm">
                ...
                <div class="space-y-4">
                    <div class="pb-4">
                        <h4 class="font-bold text-slate-700 text-sm mb-1">Bagaimana cara reschedule/mengubah jadwal?
                        </h4>
                        <p class="text-sm text-slate-500">Anda dapat membatalkan janji temu lama di menu Dashboard, lalu
                            membuat Janji Temu yang baru. Atau Anda dapat menghubungi Admin Pendaftaran.</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <footer class="mt-auto border-t border-slate-200 bg-white">
        <div
            class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex justify-between items-center text-sm text-slate-500 font-medium">
